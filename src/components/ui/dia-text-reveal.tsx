...
> {
  ...
  fixedWidth?: boolean
  /**
   * Optional manual trigger. When false, the animation will not start until `start` is true (in addition to in-view check).
   * @defaultValue `true`
   */
  start?: boolean
  /**
   * Optional array of text lines to display arranged in a column (flex-direction: column) with a synchronized sweep animation.
   */
  lines?: string[]
  /**
   * Optional flag to control whether a background layer is rendered behind the text. Defaults to false.
   * @defaultValue `false`
   */
  background?: boolean
}
...
export function DiaTextReveal({
  ...
  }, [isInView, startOnView, once, prefersReducedMotion, sweepPos, start, play])

  if (lines && lines.length > 0) {
    return (
      <div
        ref={spanRef as unknown as React.Ref<HTMLDivElement>}
        className={cn("hero-dia-lines flex flex-col text-inherit", className)}
        style={{
          display: "flex",
      ...
      >
        {lines.map((line, idx) => {
          const lineLabel = idx === 0 ? "line1" : idx === 1 ? "line2" : `line${idx + 1}`
          return (
            <div
              key={idx}
              className={cn("dia-reveal-line-wrapper", `dia-reveal-${lineLabel}-wrapper`)}
              data-line={lineLabel}
            >
              <motion.span
                className={cn(
                  "dia-reveal-line inline-block text-inherit",
                  `dia-reveal-${lineLabel}`
                )}
                data-line={lineLabel}
                style={{
